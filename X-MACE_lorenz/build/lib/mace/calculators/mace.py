...
class MACECalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        """
        Simplified calculate: dynamically handles any number of energies per atom from model output.
        Computes energies, forces, and node energies.
        """
        Calculator.calculate(self, atoms)
        batch = self._atoms_to_batch(atoms)

        # Run models and collect outputs into lists
        energy_list = []
        forces_list = []
        node_energy_list = []
        for model in self.models:
            out = model(batch.to_dict(), compute_stress=False, training=self.use_compile)
            energy_list.append(out['energy'].detach())
            forces_list.append(out['forces'].detach())
            node_energy_list.append(out.get('node_energy', torch.zeros_like(out['forces'][:,0])).detach())

        # Stack into tensors: shape [n_models, ...]
        e_tensor = torch.stack(energy_list, dim=0)
        f_tensor = torch.stack(forces_list, dim=0)
        ne_tensor = torch.stack(node_energy_list, dim=0)

        # Aggregate means and variances over models
        energy        = torch.mean(e_tensor, dim=0).cpu().numpy()
        energy_var    = torch.var (e_tensor, dim=0, unbiased=False).cpu().numpy()
        forces        = torch.mean(f_tensor, dim=0).cpu().numpy()
        forces_var    = torch.var (f_tensor, dim=0, unbiased=False).cpu().numpy()
        node_energy   = torch.mean(ne_tensor, dim=0).cpu().numpy()
        node_energy_var = torch.var(ne_tensor, dim=0, unbiased=False).cpu().numpy()
